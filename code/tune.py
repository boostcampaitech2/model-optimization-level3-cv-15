...
def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    ...
    epochs = trial.suggest_int("epochs", low=3, high=3, step=3)
    img_size = trial.suggest_categorical("img_size", [96, 112, 168, 224])
    n_select = trial.suggest_int("n_select", low=0, high=6, step=2)
    batch_size = trial.suggest_int("batch_size", low=8, high=16, step=16)
    return {
        "EPOCHS": epochs,
        "IMG_SIZE": img_size,
        "n_select": n_select,
...
def tune(gpu_id, storage: str = None):
    ...
    study = optuna.create_study(
        ...
        sampler=sampler,
        storage="postgresql://optuna:1234@127.0.0.1:6006/optuna",
        load_if_exists=True,
    )
