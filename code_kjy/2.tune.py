...
log_dir = os.environ.get("SM_MODEL_DIR", os.path.join("exp", 'optuna_latest/result_model.pt'))
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    train_loader, val_loader, test_loader = create_dataloader(data_config, isSample=True)

    criterion = nn.CrossEntropyLoss()

    # optimizer = torch.optim.SGD(model.parameters(), lr=0.1, momentum=0.9)
    optimizer = torch.optim.Adamax(model.parameters(), lr=0.01, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
...
def tune(gpu_id, storage: str = None):
    ...
    study.optimize(lambda trial: objective(trial, device), n_trials=180, gc_after_trial=True, callbacks=[lambda study, trial: gc.collect()])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", default="15", type=int, help="default seed 15.")
